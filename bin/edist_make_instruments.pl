#!/Users/drumbeat/perl5/perlbrew/perls/perl-5.18.2/bin/perl
  ...
  #
  print "\"DeviceID\",\"DeviceTerm\",\"DeviceCategory\",\"DeviceDefActions\",\"DefaultInstrumentTerm\",\"DeviceDescription\"\n";
  ...
  while (my $csv_line = <$fh>) {
	  ...
	  # print $csv_line->{EcfmID} . ":\t" . $csv_line->{EcfmName} . "\n";
          print "\"" . $csv_line->{EcfmID} . ",\"" . $csv_line->{EcfmName} . ",\"" . $csv_line->{EcfmCategory} . ",\"" . $csv_line->{EcfmDefActions} . ",\"" . $csv_line->{EcfmDefName} . ",\"" . $csv_line->{EcfmDesc} . "\"\n";
      }
      }
